...
const CreateTask: React.FC<Props> = ({user,set,refresh }) => {
  ...
  return (
    <form className="dark:bg-[#25273c] flex gap-4 p-4 rounded" onSubmit={handleOnSubmit}>
    ...
    <input className="bg-transparent outline-none" type="text" placeholder="Create a new todo..." onChange={(e) => setCreate({...create, text: e.target.value})} value={create.text}/>
  </form>
  )
}
