...
const CreateTask: React.FC<Props> = ({user,set,refresh }) => {
  const {handleClick, handleOnSubmit, create,setCreate,check,loading} = useTask(user.id, set, refresh)
  return (
    ...
    {
      !loading? <input className="bg-transparent outline-none w-[90%]" type="text" placeholder="Create a new todo..." onChange={(e) => setCreate({...create, text: e.target.value})} value={create.text}/>:
      <div className="bg-transparent outline-none w-[90%] flex items-center animate-pulse">Creating Note...</div>
    }
  </form>
  )
}
